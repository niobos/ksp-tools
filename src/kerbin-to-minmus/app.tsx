...
function App() {
    ...
    const orbitalLkoSpeed = lko.speedAtTa(0);
    ...
    const launchSite = launchSites["Kerbal Space Center"];
    ...
    const groundSpeed = 2*Math.PI * bodies.Kerbin.radius * Math.cos(launchSite.lat) / siderealDay;
    const launchDirAn = Math.asin(Math.cos(minmusInclination)/Math.cos(launchSite.lat));  // from [OMES 6.24]
    const launchSpeed = new Vector(orbitalLkoSpeed, 0, 0).rotated(new Vector(0, 0, 1), launchDirAn);
    const launchGroundSpeed = launchSpeed.sub(new Vector(0, groundSpeed, 0));
    const launchAzAn = Math.atan2(launchGroundSpeed.y, launchGroundSpeed.x);
    const launchAzDn = Math.PI - launchAzAn;

    const launchOpportunitiesJsx = [];
    for(let i=0; i<5; i++) {
        const timeAN = KerbalDateInput.formateDate(timeFirstOpportunity + i * siderealDay);
        launchOpportunitiesJsx.push(<li key={i+'an'}>{timeAN} launch azimuth {(launchAzAn/Math.PI*180).toFixed(0)}º (ENE)</li>);
        const timeDN = KerbalDateInput.formateDate(timeFirstOpportunity + i * siderealDay + siderealDay/2);
        launchOpportunitiesJsx.push(<li key={i+'dn'}>{timeDN} launch azimuth {(launchAzDn/Math.PI*180).toFixed(0)}º (ESE)</li>);
    }

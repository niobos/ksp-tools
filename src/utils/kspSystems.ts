...
export class Body {
    ...
    constructor(
        ...
        orbit: Orbit | OrbitWithoutGravity,
        other_properties: Omit<Partial<Body>, 'parentName' | 'mass' | 'radius' | 'orbit'> = {},
    ) {
        // name is filled in by KspSystem after all bodies have been constructed
        this.childrenNames = []  // filled in by KspSystem after all bodies have been constructed
        this.orbit = orbit as any as Orbit  // temporary override until KspSystem upgrades OrbitWithoutGravity to Orbit
    ...
    static massFromGASL(gASL: number, radius: number): number {
        return gASL * 9.81 * radius * radius / GRAVITATIONAL_CONSTANT
    }
        ...
        for(let bodyName in this.bodies) {
            ...
            if(body.parentName != null) {
                if(this.bodies[body.parentName] == null) {
                    ...
                    if(!('gravity' in body.orbit)) {
                        const orbitWithoutGravity = body.orbit as any as OrbitWithoutGravity
                        writableBody.orbit = Orbit.FromOrbitalElements(parent.gravity, orbitWithoutGravity.orbit, orbitWithoutGravity.phase)
                    }
...
addSystem(new KspSystem("Stock",
    {
        ...
        "Moho": new Body("Kerbol", 2.5263314e21, 250_000, {
                orbit: {sma: 5_263_138_304, e: 0.2, argp: 15/180*Math.PI, inc: 7/180*Math.PI, lon_an: 70/180*Math.PI},
                phase: {ma0: 3.14}},
            {solarDay: 2_665_723.4, atmosphereHeight: 0, terrainMaxHeight: 6817}),

        "Eve": new Body("Kerbol", 1.2243980e23, 700_000, {
                orbit: {sma: 9_832_684_544, e: 0.01, argp: 0, inc: 2.1/180*Math.PI, lon_an: 15/180*Math.PI},
                phase: {ma0: 3.14}},
        ...
        "Gilly": new Body("Eve", 1.2420363e17, 13_000, {
                orbit: {sma: 31_500_000, e: 0.55, argp: 10/180*Math.PI, inc: 12/180*Math.PI, lon_an: 80/180*Math.PI},
                phase: {ma0: 0.9}},
            {solarDay: 28_255, atmosphereHeight: 0, terrainMaxHeight: 6401}),

        "Kerbin": new Body("Kerbol", 5.2915158e22, 600_000, {
                orbit: {sma: 13_599_840_256, e: 0, argp: 0, inc: 0, lon_an: 0},
                phase: {ma0: 3.14}},
        ...
        'Mun': new Body('Kerbin', 9.7599066e20, 200_000, {
                orbit: {sma: 12_000_000, e: 0, argp: 0, inc: 0, lon_an: 0},
                phase: {ma0: 1.7}},
            {solarDay: 138_984.38, atmosphereHeight: 0, terrainMaxHeight: 7061}),
        'Minmus': new Body('Kerbin', 2.6457580e19, 60_000, {
                orbit: {sma: 47_000_000, e: 0, argp: 38/180*Math.PI, inc: 6/180*Math.PI, lon_an: 78/180*Math.PI},
                phase: {ma0: 0.9}},
            {solarDay: 40_400, atmosphereHeight: 0, terrainMaxHeight: 5700}),

        'Duna': new Body("Kerbol", 4.5154270e21, 320_000, {
                orbit: {sma: 20_726_155_264, e: 0.051, argp: 0, inc: 0.06/180*Math.PI, lon_an: 135.5/180*Math.PI},
                phase: {ma0: 3.14}},
        ...
        'Ike': new Body('Duna', 2.7821615e20, 130_000, {
                orbit: {sma: 3_200_000, e: 0.03, argp: 0, inc: 0.2/180*Math.PI, lon_an: 0},
                phase: {ma0: 1.7}},
            {solarDay: 65_517.862, atmosphereHeight: 0, terrainMaxHeight: 12738}),

        'Dres': new Body("Kerbol", 3.2190937e20, 138_000, {
                orbit: {sma: 40_839_348_203, e: 0.145, argp: 90/180*Math.PI, inc: 5/180*Math.PI, lon_an: 280/180*Math.PI},
                phase: {ma0: 3.14}},
            {solarDay: 34_825.305, atmosphereHeight: 0, terrainMaxHeight: 5700}),

        'Jool': new Body("Kerbol", 4.2332127e24, 6_000_000, {
                orbit: {sma: 68_773_560_320, e: 0.05, argp: 0, inc: 1.304/180*Math.PI, lon_an: 52/180*Math.PI},
                phase: {ma0: 0.1}},
        ...
        'Laythe': new Body('Jool', 2.9397311e22, 500_000, {
                orbit: {sma: 27_184_000, e: 0, argp: 0, inc: 0, lon_an: 0},
                phase: {ma0: 3.14}},
...
addSystem(new KspSystem("Outer Planets",
    {
        ...
        'Sarnus': new Body("Kerbol", Body.massFromGASL(0.298, 5_300_000), 5_300_000, {
                orbit: {sma: 125_798_522_368, e: 0.0534, argp: 0/180*Math.PI, inc: 2.02/180*Math.PI, lon_an: 184/180*Math.PI},
                phase: {ma0: 2.88114666938782, t0: 359.279999999964}},
        ...
        'Hale': new Body("Sarnus", Body.massFromGASL(0.0023, 6_000), 6_000, {
                orbit: {sma: 10_488_231, e: 0, argp: 0/180*Math.PI, inc: 1/180*Math.PI, lon_an: 55/180*Math.PI},
                phase: {ma0: 0, t0: 0}},
            {soi: 41_000}),
        'Ovok': new Body("Sarnus", Body.massFromGASL(0.002, 26_000), 26000, {
                orbit: {sma: 12_169_413, e: 0.01, argp: 0/180*Math.PI, inc: 1.5/180*Math.PI, lon_an: 55/180*Math.PI},
                phase: {ma0: 1.72, t0: 751.7}},
            {atmosphereHeight: 0}),
        'Eeloo': new Body("Sarnus", 1.1149224e21, 210_000, {
                orbit: {sma: 19_105_978, e: 0.0034, argp: 260/180*Math.PI, inc: 2.3/180*Math.PI, lon_an: 55/180*Math.PI},
                phase: {ma0: 3.14}},
            {atmosphereHeight: 0, terrainMaxHeight: 3900}),
        'Slate': new Body("Sarnus", Body.massFromGASL(0.692, 540_000), 540000, {
                orbit: {sma: 42_592_946, e: 0.04, argp: 0/180*Math.PI, inc: 2.3/180*Math.PI, lon_an: 55/180*Math.PI},
                phase: {ma0: 1.1, t0: 1343.91}},
            {atmosphereHeight: 0}),
        'Tekto': new Body("Sarnus", Body.massFromGASL(0.2503, 280_000), 280_000, {
                orbit: {sma: 97_355_304, e: 0.028, argp: 0/180*Math.PI, inc: 9.4/180*Math.PI, lon_an: 55/180*Math.PI},
                phase: {ma0: 2.1, t0: 1275.12}},
        ...
        'Urlum': new Body("Kerbol", Body.massFromGASL(0.257, 2_177_000), 2_177_000, {
                orbit: {sma: 254_317_012_787, e: 0.045214674, argp: 0/180*Math.PI, inc: 0.64/180*Math.PI, lon_an: 61/180*Math.PI},
                phase: {ma0: 5.59607362747192, t0: 422.539999999906}},
        ...
        'Polta': new Body("Urlum", Body.massFromGASL(0.19, 220_000), 220000, {
                orbit: {sma: 11_727_895, e: 0.0015, argp: 60/180*Math.PI, inc: 2.45/180*Math.PI, lon_an: 40/180*Math.PI},
                phase: {ma0: 1.5209, t0: 878.1399}},
            {atmosphereHeight: 0}),
        'Priax': new Body("Urlum", Body.massFromGASL(0.063, 74_000), 74_000, {
                orbit: {sma: 11_727_895, e: 0.0015, argp: 0/180*Math.PI, inc: 2.5/180*Math.PI, lon_an: 40/180*Math.PI},
                phase: {ma0: 1.5209, t0: 878.1399}},
            {atmosphereHeight: 0}),
        'Wal': new Body("Urlum", Body.massFromGASL(0.37, 370_000), 370_000, {
                orbit: {sma: 67_553_668, e: 0.023, argp: 0/180*Math.PI, inc: 1.9/180*Math.PI, lon_an: 40/180*Math.PI},
                phase: {ma0: 2.9615, t0: 1078.179}},
            {atmosphereHeight: 0}),
        'Tal': new Body("Wal", Body.massFromGASL(0.045, 22_000), 22_000, {
                orbit: {sma: 3_109_163, e: 0, argp: 0/180*Math.PI, inc: 1.9/180*Math.PI, lon_an: 40/180*Math.PI},
                phase: {ma0: 0, t0: 1179.579}},
            {atmosphereHeight: 0}),
        ...
        'Thatmo': new Body("Neidon", Body.massFromGASL(0.232, 286_000), 286_000,
            Orbit.FromOrbitalElements(14172721348500 /* filled in automatically from parent */,
        ...
        'Nissee': new Body("Neidon", Body.massFromGASL(0.045, 30_000), 30_000, {
                orbit: {sma: 487_743_514, e: 0.72, argp: 0/180*Math.PI, inc: 29.56/180*Math.PI, lon_an: 66/180*Math.PI},
                phase: {ma0: 2.04731106758118, t0: 1953406.32967385}},
            {atmosphereHeight: 0}),

        'Plock': new Body("Kerbol", Body.massFromGASL(0.148, 189_000), 189_000, {
                orbit: {sma: 535_833_706_086, e: 0.26, argp: 50/180*Math.PI, inc: 6.15/180*Math.PI, lon_an: 260/180*Math.PI},
                phase: {ma0: 0, t0: 213}},
            {atmosphereHeight: 0}),
        'Karen': new Body("Plock", Body.massFromGASL(0.066, 85_050), 85_050, {
                orbit: {sma: 2_457_800, e: 0, argp: 0/180*Math.PI, inc: 0/180*Math.PI, lon_an: 260/180*Math.PI},
                phase: {ma0: 0, t0: 213}},
            {atmosphereHeight: 0}),
    },
    "Kerbin",

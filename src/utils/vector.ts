...
export default class Vector<Type extends number=number> {
    ...
    static FromSpherical<Type extends number=number>(r: Type, i: Radians, a: Radians): Vector<Type> {
        // Inclination measured from positive z-axis
        return new Vector<Type>(
            r * Math.sin(i) * Math.cos(a) as Type,
            r * Math.sin(i) * Math.sin(a) as Type,
            r * Math.cos(i) as Type,
        )
    }

    get spherical(): [Type, Radians, Radians] {
        const xsq_plus_ysq = this.x*this.x + this.y*this.y;
        return [
            Math.sqrt(xsq_plus_ysq + this.z*this.z) as Type,  // r
    ...
    angle_to(other: Vector<Type>): Radians {
    ...
    rotated(axis: Vector, amount: Radians): Vector<Type> {
